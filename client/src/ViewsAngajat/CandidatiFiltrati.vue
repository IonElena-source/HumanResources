<template>
	<div>
		...
		>
			<CarouselCardItem
				v-for="(candidat, index) in candidati"
				:key="index"
				id="card-item"
			>
				<h1 id="titleCandidat">{{ candidat.Nume_Complet }}</h1>
				<h2 id="candidatEmail">{{ candidat.Email }}</h2>
				<h2 id="jobCandidat">{{ search(candidat) }}</h2>
				<a :href="`/files/${candidat.Cv}`" :download="`${candidat.Cv}`">
					<button id="buttonVizualizareCV" class="button is-info">
						<span id="imgEye">
				...
				<button
					@click="programeazaInterviu(candidat)"
					id="buttonProgramare"
					class="button is-info"
				>
					<span>
						<i class="far fa-clock" id="imgClock"></i>
					</span>
					Interviu
				</button>
				<button id="buttonAccepta" class="button is-success" @click="acceptaCandidat">
					<span>
				...
				<button
					...
					@click="respingeCandidat(candidat)"
				>
					Respinge
				</button>
			</CarouselCardItem>
...
export default {
	...
	methods: {
		...
		search(candidat) {
			for (let i = 0; i < this.joburi.length; i++) {
				if (this.joburi[i]._id == candidat.Id_Job) {
					return this.joburi[i].Titlu;
				}
			}
		},
		programeazaInterviu(candidat) {
	...
	mounted() {
		...
		let spanClose = document.getElementsByClassName("closeInterviu")[0];
		spanClose.addEventListener("click", () => {
			modalInterviu.style.display = "none";
		});

		axios.get("http://localhost:5000/candidati").then((response) => {
			this.candidati = response.data.candidati;
		});
...
.carousel-card-arrow-left-enter,
.carousel-card-arrow-left-leave-active {
	transform: translateY(-50%) translateX(-10px);
	opacity: 0;
}
.carousel-card-arrow-right-enter,
.carousel-card-arrow-right-leave-active {
	transform: translateY(-50%) translateX(10px);
	opacity: 0;
}
.carousel-card-item,
